...
/**
 * @Package com.cj.realtime_dws.DwsTradeProvinceOrderWindow
 * @Author chen.jian
...
//        //TODO 5.再次对流中数据进行类型转换  jsonObj->统计的实体类对象
        SingleOutputStreamOperator<TablepenviceOrderBean> beanDS = withWatermarkDS.map(
                new MapFunction<JSONObject, TablepenviceOrderBean>() {
                    @Override
                    public TablepenviceOrderBean map(JSONObject jsonObj)   {
...
//        //TODO 8.聚合
        SingleOutputStreamOperator<TablepenviceOrderBean> reduceDS = windowDS.reduce(
                new ReduceFunction<TablepenviceOrderBean>() {
                    @Override
                    public TablepenviceOrderBean reduce(TablepenviceOrderBean value1, TablepenviceOrderBean value2)   {
                ...
                new WindowFunction<TablepenviceOrderBean, TablepenviceOrderBean, String, TimeWindow>() {
                    @Override
                    public void apply(String s, TimeWindow window, Iterable<TablepenviceOrderBean> input, Collector<TablepenviceOrderBean> out)   {
