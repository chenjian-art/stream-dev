...
class SpiderCHNAmapWeatherData:
    def __init__(self, refresh_interval=500, city_result=None, process_id=None):
        ...
        self.sql_query_chn_city_all = "select code,province,city,area from " \
                                      "dev.spider_national_code_compare_dic " \
                                      "where city is not null and province <>" \
                                      " '澳门特别行政区' and province <> '台湾省' " \
                                      "and province <> '香港特别行政区';"
        if city_result is not None:
            self.city_result = city_result
        else:
            self.city_result = public_func.execute_sql(self.sql_query_chn_city_all, db_config,
                                                       as_dict=True)
        self.logger = logger

    def _refresh_keys(self):
        """刷新密钥列表"""
        self.api_keys = public_func.execute_sql(self.sql_query_amap_keys, db_config,
                                                as_dict=True)
